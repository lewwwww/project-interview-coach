...
### 🔗 与 Skill A（project-learning-coach）衔接
- **SKILL.md 阶段 1 新增输入源「学习笔记」**：若 `<你的笔记库>\学习中\<项目名>学习笔记.md` 存在（Skill A 项目学习器产物），自动读取作为项目理解素材源——含项目描述段（可直接作输入）与 12 节学习内容（提供代码锚点与业务理解），优先于从零跑代码探索；不存在则跳过，不阻塞流程
- **SKILL.md 笔记库接入新增「项目学习笔记」素材行**：实时读 `<你的笔记库>\学习中\<项目名>学习笔记.md`
- **code-evidence-map.md 输入来源新增第三项**：学习笔记 → skill 自动读（Skill A 产物），输入来源扩为 4 项（简历 / 项目描述 / 学习笔记 / 代码路径）
- 分工闭环：A 产出学习笔记 + 项目描述 → B 阶段 1 读取作为素材源 → 证据映射 → 作战卡
